    ...
    es.onmessage = (msg) => {
      ...
      switch (ev.type) {
        ...
        case 'panel.preview.ready':
          advance(ev.panelId, 'preview', ev.requestVersion, ev.assetId);
          scheduleRefetch();
          break;
        case 'panel.final.ready':
          advance(ev.panelId, 'final', ev.requestVersion, ev.assetId);
          scheduleRefetch();
          break;
        case 'panel.art.failed':
          markFailed(ev.panelId, ev.stage, ev.requestVersion);
          break;
        case 'budget.updated':
          setBudget(ev.budget);
          break;
  ...
  useEffect(() => {
    ...
    setBest((prev) => {
      ...
      for (const j of bundle.jobs) {
        ...
        if (!stage) continue;
        const cur = next[j.panelId];
        if (!cur || STAGE_IDX[stage] > STAGE_IDX[cur.stage]) {
          next[j.panelId] = { stage, requestVersion: Math.max(j.requestVersion, cur?.requestVersion ?? 0), assetId: j.assetId, failedStage: cur?.failedStage };
        }
      }
      return next;
    });
